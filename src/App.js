   ...
   const TabList = ({ tabs }) => {
      ...
      return (
         <div className='text-sm font-medium text-gray-700'>
            {tabs.length <= 2 ? (
               (tabs[0], tabs[1]) // Nếu chỉ có 1 tab, hiển thị tab đó
            ) : (
               <>
                  {tabs.slice(0, visibleTabs).join(", ")}
                     ...
                     <div className='relative'>
                        ...
                        <div>
                           {showTooltip && tabHoverId == tab.id ? (
                              <div
                                 className='absolute w-full z-50 px-2 text-xs py-2 font-normal text-black bg-custom-hover-gray transition-all duration-200 ease-[cubic-bezier(0.25, 0.1, 0.25, 1)] rounded-lg shadow-sm tooltip dark:bg-gray-700'
                                 style={{
                                    top: "calc(100% + 5px)",
                                    left: "0",
                                    transform: "none",
